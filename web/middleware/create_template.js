...
const admin_apis = async (accessToken) => {
    ...
    const getActiveThemeId = async () => {
        try {
            const response = await fetch(`https://${shopURL}/admin/api/2022-10/themes.json`, {
                method: 'GET',
                headers,
            });
            const data = await response.json();
            const activeThemeId = data.themes.find(theme => theme.role === 'main').id;
            if (activeThemeId) {
    ...
    const createFirstPage = async (templateSuffix) => {
        ...
        try {
            const response = await fetch(`https://${shopURL}/admin/api/2023-01/pages.json`, {
                method: 'POST',
                headers,
                body: pageBody,
            });
...
export default function create_template(app) {
    app.get("/api/create_template", async (req, res) => {
        try {
            ...
            const { shop } = session;
            const data = await pool.query("SELECT accesstoken FROM shopify_sessions WHERE shop=$1", [shop]);

            if (data.rows.length > 0) {
                await admin_apis(data.rows[0].accesstoken);
                return res.status(200).json({ success: true, message: data.rows[0].accesstoken });
            }
            else {
